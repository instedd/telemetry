...
class AddIndexesToTrackers < ActiveRecord::Migration
  def change
    ...
    #
    # %w(counters set_occurrences timespans).each do |tracker|
    #   table_name = "instedd_telemetry_#{tracker}".to_sym
    #
    #   add_column table_name, :key_attributes_hash, :string
    #
    ...
    #   #   WHERE id NOT IN (
    #   #     SELECT MIN(t.id)
    #   #     FROM (SELECT id, bucket, key_attributes FROM #{table_name}) AS t
    #   #     GROUP BY t.bucket, t.key_attributes
    #   #   )
    #   # SQL
    #   #
    ...
    #   # end
    #   #
    #   # add_index table_name, [:bucket, :key_attributes_hash], unique: true, name: "#{tracker}_on_bucket_and_key_attributes_hash"
    # end
  end
